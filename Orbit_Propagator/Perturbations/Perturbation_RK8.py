...
def integrate_orbit_b_with_j2(step_size=1.0, num_orbits=10, save_results=True):
    ...
    Args:
        step_size (float): Integration step size [s] (must be ≤ 1s per problem requirement)
    ...
    # Validate step size requirement
    if step_size > 1.0:
        print("WARNING: Step size > 1s violates problem requirement (Δt ≤ 1s)")
        print()
    ...
    # Theoretical comparison (approximate for high eccentricity)
    n = 2 * np.pi / orbital_period  # Mean motion
    theoretical_rate = -(3/2) * J2 * (R_EARTH_EQUATORIAL/A_ORBIT_B)**2 * n * np.cos(I_ORBIT_B_RAD)
    theoretical_shift_per_orbit = np.rad2deg(theoretical_rate * orbital_period)
    
    print("THEORETICAL COMPARISON:")
    print(f"Theoretical shift per orbit (circular approximation): {theoretical_shift_per_orbit:.6f}°")
    print(f"Simulation vs Theory ratio: {argp_shift_per_orbit_deg/theoretical_shift_per_orbit:.3f}")
    print("(Difference expected due to high eccentricity e=0.9)")
    print()
...
def plot_results(results):
    ...
    # Plot 5: Energy and Angular Momentum
    ax5 = plt.subplot(2, 3, 5)
    # Calculate specific orbital energy
    kinetic_energy = 0.5 * np.sum(state_array[:, 3:]**2, axis=1)
    potential_energy = -MU / distances
    total_energy = kinetic_energy + potential_energy
    
    ax5.plot(time_hours, total_energy, 'purple', label='Total Energy')
    ax5.set_xlabel('Time [h]')
    ax5.set_ylabel('Specific Energy [km²/s²]')
    ax5.set_title('Orbital Energy Conservation')
...
• Step size: {results['integration_params']['step_size']:.3f} s
...
• Shift per orbit: {results['argp_shift_per_orbit_deg']:.6f}°
• Total shift: {results['total_shift_deg']:.4f}°
• Theoretical (circular): {results['theoretical_shift_per_orbit_deg']:.6f}°

Physical Interpretation:
J₂ causes systematic precession of the
argument of periapsis due to Earth's
oblate shape. The effect is more
pronounced for inclined orbits.
    """
...
if __name__ == "__main__":
    ...
    # Problem requirements: Δt ≤ 1s, 10 orbits, J₂ perturbation
    print("Starting J₂ Perturbation Analysis for Orbit B")
    print("Problem: Integrate orbit with i=30° for 10 orbits including J₂ term")
    print("Requirement: Use Algorithm II (RK8) with Δt ≤ 1 s")
    print()
    
    # Run main analysis with 1.0 s step size (maximum allowed)
    results_main = integrate_orbit_b_with_j2(
        step_size=1.0,     # Maximum allowed by problem statement  
    ...
    # Additional convergence study with smaller step sizes
    print("\n" + "="*70)
    print("ADDITIONAL ANALYSIS: Step Size Convergence Study")
    print("="*70)
    print("Testing convergence with smaller step sizes for verification...")
    
    step_size_results = step_size_study(
        step_sizes=[1.0, 0.5, 0.1], 
        num_orbits=3  # Shorter for convergence test
    )
